...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%目录%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	\section{问题二模型的建立与求解}
		...
		\end{bmatrix},
		\end{gather}
		其中$t_i(i=1,2,...,n)$是极限情况时，无线充电器在传感器$s_i$处的停留时间，$c_i(i=1,2,...,n)$表示传感器$s_i$能量消耗速率，$r$是充电速率。
		...
		\begin{table}[H]
			...
			\centering		
			\caption{传感器电池最小容量分布}\label{biao1}
			\begin{tabular}{cccc}
				\toprule[2pt]
				\multicolumn{1}{m{3.5cm}}{\centering 传感器序号}
				& \multicolumn{1}{m{3.5cm}}{\centering 传感器最小电池容量}& \multicolumn{1}{m{3.5cm}}{\centering 传感器序号}& \multicolumn{1}{m{3.5cm}}{\centering 传感器最小电池容量}
				\\
				\midrule[1pt]
