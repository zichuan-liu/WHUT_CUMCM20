...
%		由表\ref{biao2}可知
	...
	\begin{figure}[H]
		\centering
		\subfigure[改变移动速度v]{\includegraphics[height=6.5cm,width=7.5cm]{2333v.jpg}}
		\subfigure[改变充电速率r]{\includegraphics[height=6.5cm,width=7.5cm]{2333r.jpg}}
	\end{figure}	
	\begin{figure}[H]
		\centering
		\subfigure[改变充电半径R]{\includegraphics[height=6.5cm,width=7.5cm]{2333R.png}}
		\subfigure[改变最低电量f]{\includegraphics[height=6.5cm,width=7.5cm]{2333f.jpg}}
    ...
    针对该多旅行商问题，我们在问题一的基础上设计了插板式编码对该分段序列类型的决策变量进行编码，并将该编码方式嵌入生命遗传算法，以总行驶路程为目标对四辆MC的充电路线进行优化求解。
      \subsection{多TSP模型建立}
  ...
  \begin{figure}[H]
  	...
  	\caption{四辆移动充电装置路径图}\label{ssssssssss}
  \end{figure}
...
移动路径（0表示数据中心）:
	...
	\setstretch{1}  %设置表的行间距
	\centering		
	\caption{每个传感器的移动路径}\label{bissssao2}
	\begin{tabular}{ccc}
		\toprule[2pt]
		\multicolumn{1}{m{2.5cm}}{\centering 传感器序号}
...
电池分布：
  	...
  	\setstretch{1}  %设置表的行间距
  	\centering		
  	\caption{传感器电池最小容量分布}\label{bissao2}
  	\begin{tabular}{cccc}
  		\toprule[2pt]
  		\multicolumn{1}{m{2.5cm}}{\centering 传感器序号}
  		& \multicolumn{1}{m{4.5cm}}{\centering 传感器最小电池容量}& \multicolumn{1}{m{2.5cm}}{\centering 传感器序号}& \multicolumn{1}{m{4.5cm}}{\centering 传感器最小电池容量}
