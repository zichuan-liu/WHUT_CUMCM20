...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%目录%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	\section{问题二模型的建立与求解}
		\subsection{问题描述与分析}
			问题二要求在问题一求得最优充电路线的基础上，给出传感器最小电池容量分布。考虑到能量得失分别与时间成正比，且是固定周期为$T$的遍历充电模型[文献10]，本节将充电周期划分为停留期和移动期。一个周期$T$内，对任意一个传感器$s_i$，移动期和所有停留期都以已知恒定速率耗电，在$s_i$停驻时以已知恒定速率充电。极限情况下，传感器能量消耗与补给的供需平衡对应传感器正常工作时的最小容量，是单值定解问题。
		\subsection{模型的建立}
			\textbf{这个地方可以细化一下}。。。。。。
			
			固定周期为$T$的遍历充电模型中，移动充电器循环遍历预先规划的路径，在预定区域向传感器充电。本节将充电周期划分为停留期和移动期，停留期移动充电器已经进入并静止于传感器强耦合磁共振圆域，完成移动充电器向目标传感器匀速充电的过程；移动期，移动充电器匀速移动，移动充电器停止供电。基于模型假设，不考虑停留期和移动期之间的时间间隔。
			为了满足使用要求，传感器电量必须始终大于阈值$f$。可知当所有传感器按最小电池容量分布时，对每个传感器$s_i$，有关于移动充电器在传感器$s_i$处停留时间$t_i$的方程组：
			\begin{gather}
			(L/v+\sum_{i=1}^{n}t_i) \cdot c_{j}=t_j \cdot r (j=1,2,...,n),
			\end{gather}
			其中，$n$是传感器数目，$L$是问题一解得最小能量消耗对应的总里程，$v$是移动充电器的移动速度，$c_{j}$是第$j$个传感器已知的能量消耗速率，$r$是充电速率。式$(1)$左右分别代表一个充电周期内消耗和补给的电量，满足传感器能量消耗与补给的供需平衡，同时保证所有传感器电量不小于$f$的使用要求。
