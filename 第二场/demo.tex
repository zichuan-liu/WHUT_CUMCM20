...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%题目%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\title{基于卷积自编码与k-medois模型\\的指纹编码与分类}
\tihao{2} 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%摘要%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	\begin{abstract}
     	指纹识别系统的存储量巨大，提高指纹的编码率具有重要意义。本文提出利用深度学习方法建立卷机自编码器模型，在非线性学习空间中有损压缩编码，并根据编码距离对指纹进行聚类。
     	
     	为消除指纹图像噪声、伪特征点的影响，设计了预处理策略体系以统一图像尺度、细化纹线骨架。首先对指纹图片进行灰度值线性归一化处理，尺寸放缩之后利用\textbf{分数阶微分算子}构造掩膜进行图像增强，最后通过方向场确定脊线以实现指纹像素二值化。预处理保留了原指纹的连接状态、拓扑结构和纹理信息，剔除了次要纹线宽度信息，有利于后续编码处理。
...
=======
		针对问题一，鉴于在小样本条件下难以有效提取指纹特征的问题，本文设计了一种\textbf{卷积自编码网络}的个体细微特征提取算法。首先将预处理后的图像信号用\textbf{卷积核}转化到高维特征空间，然后利用大量无标签的指纹高维样本训练卷积\textbf{自编码器}网络。在此基础上，通过损失函数\textbf{KL散度}对原始图像和解码后图像\textbf{无监督学习}，编码出最佳线性空间特征。经训练提取的指纹特征损失率仅占\textbf{0.04\%}，具有精校的还原程度。
>>>>>>> origin/cy
		
		针对问题二，对问题一模型生成的编码向量进行归一化处理，设计了加权相似度以量化任意图片间的相似度。首先对卷积自动编码器生成的特征编码向量进行归一化处理，即可计算任意两个编码向量间的\textbf{谷本系数}、\textbf{皮尔逊相关系数}、\textbf{马氏距离}，将三者同向化处理后加权求和即得本文设计的量化相似度。
...
=======
		针对问题三，建立中心聚类模型，以问题二中的量化相似度作为代价函数，并使用\textbf{k-medoids}算法选取聚类中心点完成样本分类；对于附件中的$16$个指纹样本，基于问题二设计的加权相似度模型可计算出每两个指纹样本间的相似度。选取$k$个样本点作为聚类中心，以样本点到聚类中心的平均值作为代价函数。遍历所有聚类中心组合，可得出被分为$k$组的样本聚类结果。$k=4$时，代价函数值最优，此时分类情况为样本$\bm{10}$独立为一组，其余三组是\textbf{$\bm{\left \{ 2,3,5   \right \}}$}，\textbf{$\bm{\left \{ 1,4,7,11,16 \right \}}$}与 \textbf{$\bm{\left \{ 6,8,9,12,13,14,15 \right \}}$}。
>>>>>>> origin/cy
		
		
		本文的优点为：1. 采用卷积核函数提取指纹细节，可充分压缩图像，在大幅度降低数据维度的同时尽可能挖掘图像细节特征。2.于传统编码而言，具有相当强的个体表征能力，可以通过解码器还原出完整度相当高的原图片。
...
其中255为二值图像中图像背景和谷线的灰度值，0 为二值图像中图像脊线的灰度值。
	...
	\section{问题一模型的建立与求解}
		...
		\subsection{问题描述与分析}
			问题一要求给出一种用不超过200字节来刻画描述指纹基本特征的表示方法。传统的图像压缩方法有霍夫曼压缩、Golomb编码、LZW编码、小波编码等\upcite{7,8,9}，其都能对指纹进行刻画。但当编码字节大小限制在200之内时，传统方法在压缩编码方面都显得不尽人意。
			
			由于指纹图像细节信息较为丰富，使用传统方式编码将导致大量细节信息损失，而相较于传统的图片编码而言，网络编码能更有效的处理图像的细节信息。在训练过程中，自编码器通过无监督学习识别判断并提取的图像关键信息并将其编码，可使得几乎完整的图片信息被保存于200字节以内的编码中。自编码器仅提取对于网络的有效信息，即降低了数据维度，从而实现小样本编码。
...
设计加权相似度以量化特征向量间的相似程度。首先对各行向量的每一维度进行归一化处理，之后分别计算归一化后的行向量间的谷本系数(Tanimoto Coefficient)、皮尔森相关系数(Pearson Correlation Coefficient)和马哈拉诺比斯距离(Mahalanobis Distance)，最后构造关系函数确定行向量的相似程度。
    ...
    \subsection{灵敏度分析}
    由可视化结果可知，图(a)深色和浅色区域范围离散化程度较大,图(b)、(c)中深色区范围较大，区分度较差，图(d)浅色区域占比略大于深色区域，深色区域集中分布，区分度和辨识度较好，可知相似度函数最优的权重分配应为$\omega_1=0.2,\omega_2=0.3,\omega_3=0.5$。
        \begin{figure}[H]
    	\centering
    ...
    本文设计的加权相似度$S$具有非欧几里得特征\upcite{10}，不可使用样本均值作为聚类中心，即不可使用经典中心聚类算法$k-means$算法经行样本聚类。针对该问题，我们引入$k-medoids$算法以确定样本中心点，并以模型二设计的加权相似度的平均值作为代价函数，对附件中的$16$个指纹样本进行聚类。
  ...
  J(c_1,c_2,\cdots,c_k)=\frac{1}{16}\sum _{i=1}^{16}\underset{1\leqslant j\leqslant k}{Max}S(\gamma _i,c_j),
      ...
      \end{gather}
  	鉴于该问题样本规模较小，本文采用遍历方式，输入所有聚类中心并选取代价函数值最高的聚类中心组合，其伪代码如下所示
  	
  	\begin{algorithm}[H]
  		\caption{Procedure of k-medoids Algorithm}
  		\setstretch{.8}  %设置表的行间距
  		\LinesNumbered
  		\KwIn{
  			Number of centers: $k$ 
  		}
  		\KwOut{Centers:$\left \{ c_i \right \}$}
  		\textbf{Initialize} $J_{max}\leftarrow 0$ \newline
  		\For{  $\pi_1$  = 1 \rm to 16 \rm-k}{
  		  \For{  $\pi_2$ =2 \rm to 17 \rm-k}{  
  				$\vdots$\\ 
  				 \For{  $\pi_k$ =k \rm to 16\rm}{
  				 	\If{$J(\gamma_{\pi_1}, \gamma_{\pi_2},\cdots,\gamma_{\pi_k})>J_{max}$}{
  				 	$J_{max}=J(\gamma_{\pi_1},\gamma_{\pi_2},\cdots,\gamma_{\pi_k})$
  			 	}
  		     }
  	       	$\vdots$\\ 		 
  		  }
  	...
  	12,13,14,15 \right \}$两组；当$k=4$时，样本$10$独立为一组，其余三组是$\left \{ 2,3,5   \right \}$，$\left \{ 1,4,7,11,16 \right \}$与 $\left \{ 6,8,9,12,13,14,15 \right \}$。鉴于$k=4$时，代价函数值最优，本文倾向于将样本分为四类如下图所示，并分别命名为：螺形纹
  	、环形纹、弓形纹、箕形纹。
  	  	\begin{figure}[H]
  		\centering
  	...
  	\section{模型的评价}
		\subsection{模型的优点}
			...
			\item [(2)] 于传统编码而言，CAE模型用于提取“小样本”条件下相同类型、相同纹理的指纹特征，获得在“小样本”条件下超越传统方法的编码个体表征能力，并且在编码后能通过解码器还原出原始图像数据。
			\end{itemize}
		\subsection{模型的缺点}
