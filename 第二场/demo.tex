...
\end{figure}
	...
	\section{问题一模型的建立与求解}
	   ...
	   \begin{gather*}
		...
		对$\forall \alpha \in\mathbb{R}$，定义Grunward-Letnikov分数阶微分算子
		\begin{align}
		...
		\end{gather}
		其中，$p(x_i)$是事件$x_i$发生的概率，$n$是可能发生事件的个数。对信息熵进行归一化处理，可以获得取值范围为$[0,1]$的信息熵$\displaystyle E'=\frac{E-E_{min}}{E_{max}-E_{min}}$。
