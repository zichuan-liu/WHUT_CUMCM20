...
\end{figure}
	...
	利用Grunward-Letnikov微分算子，构造自适应函数，并将其应用于指纹像素掩膜；采集梯度信息和计算信息熵，从而确定微分阶数。本方法非线性加强了像素信息中的高频成分，也保留了一定的低频和直流成分，有效提高图像质量，减少掩膜耗时。
		...
		\end{pmatrix}f(x_i-k) 
		\end{gather}
		其中，$x_i(i=0,1,...,6)$是不同的偏微分方向，此处取以$x$轴正向为起始方向，顺时针旋转一周均匀分部的8个方向。为方便计算，此处$x_i-v$取$3$。分别在$8$个方向采用分数阶微分近似掩膜，从而提高图像质量。
